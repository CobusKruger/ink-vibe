...
final class Hub {
	 ...
	 *
	 * The `s` keyword is added only for a non-empty trimmed term — so a hostile/
	 * garbage query string degrades to the unfiltered newest-first listing rather
	 * than a broken query. (The `vaardigheid` facet is Story 11.2.)
	 *
	 ...
	 */
	public static function queryArgs( int $paged, int $per_page, string $search = '' ): array {
		$args = array(
	...
	public static function render(): string {
		...
		$query = new \WP_Query( self::queryArgs( $paged, self::PER_PAGE, $search ) );
		...
		// Featured strip only on the unfiltered first page (no search/paging).
		$featured = array();

		if ( 1 === max( 1, $paged ) && '' === $search ) {
	 ...
	 * @param array{paged:int, max_pages:int, search?:string}            $nav      Render context.
	 * @return string
	 */
	public static function toHtml( array $cards, array $featured, array $nav ): string {
