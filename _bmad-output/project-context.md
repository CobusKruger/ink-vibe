...
- **Brain Monkey test-isolation patterns.** The unit suite mocks WordPress, so shared/static state leaks across tests unless reset. (a) **Reset first-wiring-wins facade statics** (`??=`-initialised singletons/maps) via reflection in `beforeEach`, so one test's wiring doesn't bleed into the next. (b) **Prefer a protected overridable seam** (e.g. `buddyPressActive()`, `gradingValueFor()`, `entryCountFor()`) over an inline `function_exists()`/static call for any guard or external read a test needs to vary — subclass-and-override beats global stubbing. (c) **WP constants, output-format flags, and `WP_*` class doubles live in `tests/bootstrap.php` + `tests/stubs/`** (the `WP_User`/`WP_Error`/`WP_Term`/`WP_Post` convention), never redefined per-test. Promoted from candidate (recurred 7.x, 9.x) before Epic 12A's stateful ingestion + facades.
- **Test the OUTCOME, not the arg-shape.** Assert INK-owned behaviour and INK-owned data (the `WP_Query` args *we* build, the meta *we* write, the markup *we* emit), never a third-party contract's internal shape read back through a mock — a mock asserting a mock proves nothing and breaks on the dependency's upgrade. Pure `queryArgs()`/`toHtml()`/`arrange()` returning INK-defined structures are the acceptable case; asserting WooCommerce/BuddyPress internal call shapes is not. Promoted from candidate before Epic 12A.
