...
2. **It is large and multi-domain with cross-cutting concerns.** Eighteen epics span 9 CPTs, commerce, community, accounts/auth, a custom submission workflow, reading engagement, migration, and an Afrikaans-first mandate that touches every surface. Concerns like Afrikaans-first, the three-layer architecture, and migration cut *across* features rather than living inside one slice. BMAD's PRD + architecture-then-shard structure is built to hold cross-cutting decisions once and decompose downward; Spec-Kit's per-feature specs tend to duplicate or lose cross-cutting context.
...
6. **Epic/story sharding** → use `ink-feature-list.md`'s 18 epics directly (listed in build/dependency order); each feature row is a story seed with layer, data source, and acceptance hints already attached. Prioritise P0 → P1 → P2.
...
8. **Sequence around migration** → treat Epic 16 (migration & redirects) as a release-gating track running alongside feature epics, per the migration order in §11.
